...
%         %% RJC
%         fprintf(fid,'/FILNAME,''%s'',1\n',strcat(ansysFilename, int2str(iLoad)));   %From master, change the jobname
%         %% End RJC
        %fprintf(fid,'resume\n');
        ...
        if isfield(config.ansys.analysisFlags,'StaticNonlinear') && config.ansys.analysisFlags.StaticNonlinear == 1
        ...
        %% ************************************************************************
        %================= PERFORM LINEAR LOCAL BUCKLING ANALYSIS =================
        if isfield(config.ansys.analysisFlags,'localBuckling') && ~isempty(config.ansys.analysisFlags.localBuckling)&& ~(isfield(config.ansys.analysisFlags,'imperfection') && ~isempty(config.ansys.analysisFlags.imperfection))
            ...
            %Calculate face stresses and check for wrinkling here in a linear analysis
            [app,SkinAreas,compsInModel]=writeANSYSgetFaceStresses(blade,fid,config.ansys.analysisFlags.localBuckling);
            
        end
        
        %% ************************************************************************
        % ================= OUTPUT RESULTANT VS SPAN ANALYSIS =================
        ...
        %% ************************************************************************
        % ================= CREATE LOCAL FIELD RESULTS FOR MATLAB =================
...
%% %%%%%%%%%%%%%%%% MATLAB POST PROCESS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if isfield(config.ansys.analysisFlags,'deflection') && config.ansys.analysisFlags.deflection~=0
            ...
            designvar = ReadANSYS_DeflectionResult(blade, config, iLoad, fid, deflectionFilename);
        ...
        % at the end of the nonlinear analysis for wrinkling check 
        if isfield(config.ansys.analysisFlags,'imperfection') && ~isempty(config.ansys.analysisFlags.imperfection)
            ...
            for jj=1:length(imperfection)
                for ii=1:length(linearLoadFactors) 
                   ...
                   nonlinearLoadFactors(ii,jj)=nonlinearBuckling(ansysFilename, ansys_path, ansys_product, config, ii, jj, ncpus);
        ...
        %% ************************************************************************
        % ================= POST-PROCESS PANEL WRINKLING FACTORS (CURRENTLY NON-FUNCTIONING AND UNSUPPOTED) =================
